...
function ledGroup({
  ...
  return `<g transform="translate(${x} ${y}) scale(${scale})">
    ...
    ${aura ? `<rect x="33" y="18" width="30" height="60" rx="15" fill="${aura}" opacity=".18"/>` : ''}
...
async function renderBanner(config, frame = null) {
  ...
  const overlayMarkup = config.textRenderer({
    ...
    height: config.height,
    ...config.text,
    logoMarkup: `${config.text.logoExtra ?? ''}`,
  });
...
async function generateLogos() {
  const variants = [
    ...
    },
    {
      ...
      baselineFill: 'rgba(246,243,236,.72)',
      baseline: true,
...
async function generateStaticBanners() {
  ...
  const configs = [
    ...
    },
    {
      ...
      height: 1080,
      background: COLORS.ink,
      photo: { file: PHOTO.led, position: 'centre' },
      overlayGradient: {
        stops: [
    ...
    },
    {
      ...
      overlayGradient: {
        stops: [
          { offset: '0%', color: '#0F1722', opacity: 0.28 },
          { offset: '100%', color: '#0F1722', opacity: 0.86 },
        ],
      },
      textRenderer: renderLinkedInSplit,
      mark: { x: 74, y: 70, size: 74, ...commonMark },
      text: {
    ...
    },
    {
      ...
      background: COLORS.ink,
      textRenderer: renderLinkedInSplit,
      mark: { x: 1230, y: 104, size: 148, tileFill: COLORS.ink, shell: 'rgba(246,243,236,.22)', core: COLORS.lake },
      text: {
        kicker: 'Disconnect to reconnect',
        kickerSize: 15,
        kickerTracking: 5,
        ...
        bodyY: 282,
        accentPill: { x: 1198, y: 272, width: 210, height: 44, fill: 'rgba(246,243,236,.08)', label: 'page cover', labelFill: COLORS.porcelain },
      },
    },
    {
      ...
      textRenderer: renderLinkedInSplit,
      mark: { x: 78, y: 60, size: 66, ...commonMark },
      text: {
        kicker: 'faradaybox',
        kickerSize: 15,
        kickerTracking: 5,
        kickerFill: 'rgba(246,243,236,.56)',
        padding: 78,
        lines: ['Déposez le téléphone.', 'Gardez le moment.'],
        titleSize: 60,
        lineHeight: 72,
        titleFill: COLORS.porcelain,
        titleY: 192,
        bodyLines: ['Premium, discret, immédiatement compréhensible.'],
        bodySize: 22,
        bodyLineHeight: 34,
        bodyFill: 'rgba(246,243,236,.76)',
        bodyY: 314,
    ...
    },
    {
      ...
      background: COLORS.sage,
      textRenderer: renderLinkedInSplit,
      mark: { x: 1260, y: 92, size: 162, tileFill: COLORS.sage, shell: 'rgba(21,29,42,.15)', core: COLORS.porcelain },
      text: {
    ...
    },
    {
      ...
      overlayGradient: {
        stops: [
          { offset: '0%', color: '#0F1722', opacity: 0.74 },
          { offset: '100%', color: '#0F1722', opacity: 0.9 },
        ],
      },
      textRenderer: renderLinkedInSplit,
      mark: { x: 78, y: 64, size: 66, ...commonMark },
      text: {
        kicker: 'Disconnect to reconnect',
        kickerSize: 15,
        kickerTracking: 5,
